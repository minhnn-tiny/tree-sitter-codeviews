...
class SolidityParser(CustomParser):
    ...
    def create_all_tokens(self, src_code, root_node, all_tokens, label, method_map, start_line):
        ...
        # print('root node: ', type(root_node), root_node)
        # print(f'node info: {root_node.type} - {root_node.is_named} - {root_node.text} - {root_node.start_point}-{root_node.end_point}')
        # if root_node.type == 'if_statement':
        #     print('root node here : ', root_node.children)
