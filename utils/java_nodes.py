...
def get_nodes(root_node=None, node_list={}, graph_node_list=[], index={}, records = {}, statement_types = {}):
    ...
    if root_node.type == 'parenthesized_expression' and root_node.parent is not None and root_node.parent.type == 'do_statement':
        ...
        graph_node_list.append((index[(root_node.start_point,root_node.end_point,root_node.type)], root_node.start_point[0], label, type_label, method_sig))
    ...
    #             print("MARKER", root_node.start_point)
        ...
        if root_node.type in statement_types['inner_node_type'] and root_node.parent is not None and root_node.parent.type in statement_types['outer_node_type'] and root_node.parent.child_by_field_name('body') != root_node:
            ...
            label = root_node.text.decode('UTF-8')[:20]
            ...
            if root_node.type == 'method_declaration' or root_node.type == 'constructor_declaration':
                ...
                label = method_name[0].text.decode('UTF-8') +para_list[0].text.decode('UTF-8')
                ...
                records['method_locations'][method_sig] = [root_node.start_point, root_node.end_point]
                graph_node_list.append((index[(root_node.start_point,root_node.end_point,root_node.type)], method_name[0].start_point[0], label, type_label, method_sig))
                ...
                print('Added for node: ', root_for_index)
                ...
                type_label = 'label'
            elif root_node.type == 'local_variable_declaration' or \
                 root_node.type == 'variable_declarator':
                # label = 'new_variable'
            ...
            if root_node.type != 'method_declaration' and root_node.type != 'constructor_declaration':
                ...
                if root_node.type == 'if_statement':
                    ...
                    graph_node_list.append((records['end_if_node'][root_if_index][0], records['end_if_node'][root_if_index][1][0], 'end_if', 'end_if', method_sig))
                elif root_node.type == 'for_statement' or root_node.type == 'enhanced_for_statement':
                    graph_node_list.append((records['end_loop_node'][root_for_index][0], records['end_loop_node'][root_for_index][1][0], 'end_loop', 'end_loop', method_sig))
                elif root_node.type == 'while_statement':
                    graph_node_list.append((records['end_loop_node'][root_while_index][0], records['end_loop_node'][root_while_index][1][0], 'end_loop', 'end_loop', method_sig))
