...
class ASTGraph:
    ...
    def get_AST_nodes(self, root_node, AST, AST_index):
        ...
        if root_node.is_named:
            current_node_id = AST_index[(root_node.start_point,root_node.end_point, root_node.type)]
    ...
    def minimize(self, root_node, blacklisted_nodes):
        if root_node.type in self.properties.get('blacklisted', []):
            blacklisted_nodes.append(self.index[(root_node.start_point,root_node.end_point,root_node.type)])
